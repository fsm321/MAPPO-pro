...
class EntityState(object):
    def __init__(self):
        ...
        self.roll = 0.0  # Roll state for normalized 3-DOF control
        self.goal = ""
        self.c = ""
...
class Action(object):
    def __init__(self):
        ...
        # Physical action = [n_x, n_z, roll_cmd].
        self.u = np.zeros(3)
...
class World(object):
    ...
    def _legacy_update_agent_state(self, agent):
    ...
    # Override the legacy direct-rate controller with normalized 3-DOF control.
    def update_agent_state(self, agent):
        action = np.asarray(agent.action.u, dtype=float)

        # Use normalized 3-DOF controls without switching to real metric units.
        g_eff = 1.0
        dt = self.dt

        # action[0] -> tangential control n_x
        # action[1] -> normal control n_z
        # action[2] -> roll command phi
        n_x = 2.0 * np.clip(action[0], -1.0, 1.0)
        n_z = 1.0 + 3.0 * np.clip(action[1], -1.0, 1.0)
        roll_cmd = (np.pi / 2.0) * np.clip(action[2], -1.0, 1.0)

        # First-order roll response avoids discontinuous heading changes.
        roll_rate_limit = np.pi / 2.0
        roll_error = roll_cmd - agent.state.roll
        roll_step = np.clip(roll_error, -roll_rate_limit * dt, roll_rate_limit * dt)
        agent.state.roll += roll_step
        agent.state.roll = np.clip(agent.state.roll, -np.pi / 2.0, np.pi / 2.0)

        v = max(agent.state.p_vel, 0.3)
        gamma = agent.state.pitch
        phi = agent.state.roll

        v_dot = g_eff * (n_x - np.sin(gamma))
        pitch_dot = (g_eff / v) * (n_z * np.cos(phi) - np.cos(gamma))

        cos_gamma = max(abs(np.cos(gamma)), 0.2)
        yaw_dot = (g_eff * n_z * np.sin(phi)) / (v * cos_gamma)

        # Keep the original asymmetry: blue team remains slightly weaker.
        if agent.team == 1:
            yaw_dot *= 0.8
            v_max = 2.5
        else:
        ...
        agent.state.p_vel = np.clip(agent.state.p_vel + v_dot * dt, 0.5, v_max)
        agent.state.pitch = np.clip(agent.state.pitch + pitch_dot * dt, -np.pi / 3, np.pi / 3)
        agent.state.yaw += yaw_dot * dt
        agent.state.yaw = (agent.state.yaw + np.pi) % (2 * np.pi) - np.pi

        vx = agent.state.p_vel * np.cos(agent.state.yaw) * np.cos(agent.state.pitch)
        vy = agent.state.p_vel * np.sin(agent.state.yaw) * np.cos(agent.state.pitch)
        vz = agent.state.p_vel * np.sin(agent.state.pitch)

        agent.state.p_pos[0] += vx * dt
        agent.state.p_pos[1] += vy * dt
        agent.state.z_pos += vz * dt
